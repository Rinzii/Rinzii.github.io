...
function calculateDropdownBottom() {
    ...
    if (dropdownItems.length === 0) {
        return height; // Default to full height if no dropdown exists
    }

    var lastItem = dropdownItems[dropdownItems.length - 1];
    var rect = lastItem.getBoundingClientRect();
    return rect.bottom + window.scrollY; // Adjust relative to document
}
...
function Terrain(options) {
    ...
    // Dynamically set maxHeight based on dropdown position
    this.maxHeight = calculateDropdownBottom() - 10; // Add some padding
    this.mHeight = Math.min(options.mHeight || height, this.maxHeight);
...
// Recalculate mountain heights on window resize
function updateTerrainMaxHeight() {
    entities.forEach(entity => {
        if (entity instanceof Terrain) {
            entity.maxHeight = calculateDropdownBottom() - 10; // Recalculate dynamically
...
window.addEventListener("resize", updateTerrainMaxHeight);
